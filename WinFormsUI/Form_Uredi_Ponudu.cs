...
{
    public partial class Form_Uredi_Ponudu : Form
    {
        #region Varijable
        Form_Main main;
        ...
        List<DetaljiPonudeModel> detalji = new List<DetaljiPonudeModel>();
        ...
        public Form_Uredi_Ponudu(string br, Form_Main f)
        {
            InitializeComponent();
            brojPonude = br;
            main = f;
        ...
        private void Form_Nova_Ponuda_Load(object sender, EventArgs e)
        {
            ...
            detalji = SqliteDataAccess.UcitajDetaljePonude();
            ...
            {
                if(ponuda.BrojPonude.ToString() == brojPonude)
                {
                    SIFRA_KLIJENTA = ponuda.SifraKlijenta;
                    Input_Datum.Text = ponuda.Datum;
                    Input_Naslov.Text = ponuda.Naslov;

                    Input_ImePrezimeNaziv.Text = ponuda.Klijent;
                    Input_Adresa.Text = ponuda.Adresa;
                    Input_Posta.Text = ponuda.Posta;
                    Input_Oib.Text = ponuda.Oib;
                    Input_Dodatno.Text = ponuda.Dodatno;
                    ...
                    {
                        Input_Rabat.Text = ponuda.Rabat;
                    }
                    
                    Input_Napomena.Text = ponuda.Napomena;
                }
            }

            foreach(var detalj in detalji)
            {
                if(detalj.BrojPonude.ToString() == brojPonude)
                {
                    Card c2 = new Card();
                    ...
                    foreach (TextBox t in c2.Controls.OfType<TextBox>())
                    {
        ...
        {
            if (MessageBox.Show("Želite li poništiti uređivanje ponude? Podaci ponude se neće mijenjati.", "Upozorenje", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Close();
            }
        }
        ...
        private void Button_Spremi_Click(object sender, EventArgs e)
        {
            ...
            #region Brisanje pa spremanje podataka u bazu podataka
            IzbrisiPonudu(brojPonude);
        ...
        private void IzbrisiPonudu(string br)
        {
            SqliteDataAccess.IzbrisiPonudu(br);
        }
